...
class Feature:
    ...
    def __init__(self, filename, directory, number, feature, lidar_filename, lidar_img, EPSG):  # noqa:N803
        ...
        Args:
            filename: name of the file within the directory in question
    ...
    def tree_height(self):
        """Crops the lidar tif to the features and calculates the 95% greatest height to account for error at the top end.
...
def feat_threshold_tests(feature_instance, conf_threshold, area_threshold, border_filter, tile_width):
    ...
    tile that have a bit spilt over.  Removes features within a border of the edge, border size is given by
