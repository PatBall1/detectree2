...
<<<<<<< HEAD
    ...
    """Tiles up orthomosaic into managable chunks to make predictions on.
...
=======
              crowns: str = None) -> None:
    """
    Tile up image and (if included) corresponding crowns
    """
    for minx in np.arange(data.bounds[0], data.bounds[2] - tile_width, tile_width,
                          int):
        for miny in np.arange(data.bounds[1], data.bounds[3] - tile_height,
...
>>>>>>> 150cc12 (apply autopep8 to tiling.py)
...
<<<<<<< HEAD
            ...
            # print("Coords:", coords)
=======
            # define the bounding box of the tile, excluding the buffer (hence
            # selecting just the central part of the tile)
            bbox_central = box(minx, miny, minx + tile_width, miny + tile_height)
...
=======
            # stack up the bands in an order appropriate for saving with cv2, then
            # rescale to the correct 0-255 range for cv2
...
=======
            ...
            if crowns is not None:
                ...
                # this causes an error so skip creating geojson. The training code will
                # also ignore png so no problem.
                try:
...
=======
    # Right let's test this first with Sepilok 10cm resolution, then I need to
    # try it with 50cm resolution.
>>>>>>> 150cc12 (apply autopep8 to tiling.py)
...
=======
    # resolution = 0.6 # in metres per pixel - @James Ball can you get this
    # from the tiff?
>>>>>>> 150cc12 (apply autopep8 to tiling.py)
